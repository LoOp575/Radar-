              ...
              <table className="w-full min-w-[920px] border-separate border-spacing-y-1.5 text-left font-mono text-xs">
                ...
                <tbody>
                  {signals.map((token, index) => (
                    ...
                    >
                      <td className="rounded-l-xl px-3 py-3 font-bold text-slate-500">
                        #{index + 1}
                      </td>
                      ...
                      <td className="px-3 py-3">
                        <span
                          className={`rounded-md px-2 py-0.5 font-black ring-1 ${ppsBadge(token.pps)}`}
                        >
                          {token.pps}
                        </span>
                      </td>
                      <td className="px-3 py-3">
                        <span
                          className={`rounded-md px-2 py-0.5 font-black ring-1 ${riskBadge(token.risk)}`}
                        >
                          {token.risk}
                        </span>
                      </td>
                      <td className="px-3 py-3">
                        <span
                          className={`whitespace-nowrap rounded-md px-2 py-0.5 text-[10px] font-bold ring-1 ${signalBadge(token.signal)}`}
                        >
                          {token.signal}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-slate-300">
                        ${formatCompact(token.volume15m)}
                      </td>
                      <td
                        className={`px-3 py-3 font-black ${token.oiChange1h >= 0 ? 'text-emerald-200' : 'text-red-200'}`}
                      >
                        {token.oiChange1h >= 0 ? '+' : ''}
                        {token.oiChange1h}%
                      </td>
                      <td className="px-3 py-3 text-emerald-200">
                        ${formatCompact(token.smartWalletNetBuyUsd)}
                      </td>
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    {topSignal.name} · {topSignal.chain}
                  </p>
                </div>
                <span
                  className={`rounded-md px-2 py-1 text-[10px] font-black ring-1 ${signalBadge(topSignal.signal)}`}
                >
                  {topSignal.signal}
                </span>
              </div>
              ...
              <div className="mt-4">
                ...
                <ul className="space-y-1.5 text-[11px] text-slate-300 md:text-xs">
                  {topSignal.reasons.slice(0, 4).map((reason) => (
                    <li
                      key={reason}
                      className="flex items-start gap-2 rounded-lg bg-emerald-300/5 px-2.5 py-1.5 ring-1 ring-emerald-300/15"
                    >
...
function Metric({
  ...
  return (
    <div
      className={`rounded-xl border border-white/10 bg-gradient-to-br ${toneClass} to-white/[0.025] p-3 shadow-xl shadow-black/20 backdrop-blur`}
    >
    ...
    <div className="rounded-xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-white/[0.02] p-2.5 shadow-lg shadow-black/10">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          ...
          <span
            className={`whitespace-nowrap rounded-md px-2 py-0.5 text-[9px] font-bold ring-1 ${signalBadge(token.signal)}`}
          >
            {token.signal}
          </span>
        </div>
      ...
      <div className="mt-2 flex items-center justify-between rounded-lg bg-black/25 px-2.5 py-1.5 text-[10px] text-slate-400 ring-1 ring-white/10">
        <span>
          Smart: <b className="text-emerald-200">${formatCompact(token.smartWalletNetBuyUsd)}</b>
        </span>
        <span>
          OI 1h: <b className={token.oiChange1h >= 0 ? 'text-emerald-200' : 'text-red-200'}>
            {token.oiChange1h >= 0 ? '+' : ''}
            {token.oiChange1h}%
          </b>
        </span>
